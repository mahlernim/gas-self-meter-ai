...
The next test release uses application ID `dev.mahlernim.gasselfmeter`, version name `0.3.1`, and version code `8`. Supported devices run Android 8.0 or later. The build targets Android 16 / API 36. Distribution is limited to the Google Play Closed Alpha track. Do not attach an APK or AAB to a GitHub release.
...
The signing script passes the password through environment variables so it does not appear as a command-line argument. It generates `artifacts/gas-self-meter-ai-0.3.1.aab`. Upload this AAB only to the app's Closed Alpha track in Google Play Console. Do not distribute it from GitHub or another public download page.
