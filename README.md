...
별도 센서나 개발자 서버 없이 계량기 지침을 추정하고, 검침 기간에 맞춰 부산도시가스에 검침값을 직접 또는 자동으로 입력하는 Android 앱입니다. 작년 같은 달과 앞뒤 달의 사용 흐름에 최근 실측값을 반영해 오늘의 누적 지침을 계산합니다.
...
- 부산도시가스의 검침 가능 기간, 입력 대상 여부와 기존 입력 상태 확인
...
자동 입력은 단순한 알림이나 로컬 기록 기능이 아닙니다. 앱이 계산한 누적 지침을 실제 부산도시가스 자가검침 기록에 입력합니다. 기본값은 꺼짐이며 사용자가 제출 탭에서 명시적으로 켜야 합니다.
...
계정과 생활 기록은 Android Keystore로 보호되는 키를 사용해 기기에 암호화해 저장합니다. 부산 연결 시 기기에서 공급사 HTTPS 서버로 직접 로그인하고 조회하며, 사용자가 켠 경우 검침값을 공급사로 직접 전송합니다. 개발자 서버, 광고, 분석 SDK, 외부 AI API가 없습니다. 백업에는 비밀번호와 아이디를 포함하지 않습니다. 기기를 바꾸거나 앱을 삭제하기 전 백업을 내보내 주세요.
