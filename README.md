...
- 검침 화면의 우리 집 정보 카드에서 조회된 계약번호, 계량기번호와 주소를 확인하고 각각 복사할 수 있습니다. 표시 항목은 공급사와 저장된 정보에 따라 달라집니다.
...
구현된 기능을 지원으로 표시하고, 실계정 검증 여부는 별도로 안내합니다. 아래는 소스 버전 0.4.2 기준이며 배포된 앱의 버전에 따라 다를 수 있습니다.

| 공급사 또는 채널 | 조회·기록 지원 | 앱에서 직접 제출 | 마지막 날 자동 제출 |
| --- | --- | --- | --- |
| 부산도시가스 | 로그인·청구 이력 가져오기 | 지원 | 조건부 지원 |
| 코원·충청·영남 구미·포항·전남도시가스·강원도시가스·전북에너지서비스 | 로그인·청구 이력 가져오기 | 지원 | 미지원 |
| 서울·예스코·인천 등 가스앱 14개 공급사 | 본인인증·청구 이력 가져오기 | 지원 | 조건부 지원 |
| 삼천리 | 알파 로그인·청구 이력 가져오기 | 미지원 | 미지원 |
| 경남에너지 고객번호 조회 | 실험실에서 청구 정보 참고 표시 | 미지원 | 미지원 |
| 대성에너지·대성청정에너지 | 실험실에서 로그인 폼·요금 페이지 구조 점검 | 미지원 | 미지원 |
| EnergyTalk | 공식 웹 로그인 후 실험실에서 사용량·자가검침 상태 참고 표시 | 앱 자체 제출 미지원 | 미지원 |
| 모든 공급사 | 앱 내 수동 기록과 추정 | 위 공급사별 범위 적용 | 위 공급사별 범위 적용 |

실험실 결과는 앱의 청구 이력이나 추정 모델로 가져오지 않습니다. 전체 공급사 목록, 화면 연결과 검증 상태는 [공급사 지원 범위](docs/PROVIDERS.md)를 참고하세요.
...
0.4.1에서는 조회 취소와 피드백 초안 공유를 추가하고 제출·저장·청구 이력 처리를 개선했습니다. 알파에서는 합리적인 가정을 문서화해 연결을 먼저 제공하고, 실제 사용 피드백으로 고쳐 나갑니다. [0.4.1 변경 사항과 피드백 방법](docs/ALPHA_0.4.1.md)을 확인해 주세요.

첫 시작 화면과 설정에서 `알파 연결 실험실`을 열 수 있습니다. 본인 계정이나 조회 권한이 있는 고객번호로 매번 동의한 뒤 실행합니다.

- 경남에너지 고객번호로 고지월, 금액, 보정사용량과 사용열량을 참고용으로 조회합니다. 보정사용량은 계량기 지침 차이와 다를 수 있어 추정 이력에 가져오지 않습니다.
- 대성에너지와 대성청정에너지의 공식 로그인 폼과 월별 요금 페이지 구조를 점검합니다. 세션과 비슷한 화면 구조는 실제 인증 성공이나 청구 이력 연동 완료를 뜻하지 않습니다.
- 에너지톡 11개 서비스 구분에서 공식 웹 로그인과 주소 선택 후 사용량·자가검침 상태를 앱 화면에 참고용으로 표시합니다. 참빛 지역·지점을 포함한 서비스 구분이며 11개 독립 공급사나 검증 완료 연동을 뜻하지 않습니다.

새 실험의 실제 계정 성공은 아직 검증되지 않았습니다. 결과를 내 기록, 추정 모델과 백업에 합치거나 앱에서 검침값을 제출하지 않으며 기존 백그라운드 조회·제출 설정도 바꾸지 않습니다.

에너지톡 공식 웹 화면은 조회 전용이 아닙니다. 가입, 약관 동의, 결제와 검침 제출이 가능하므로 실험 중에는 결제·제출 버튼을 누르지 마세요. 정상 종료 시 에너지톡 저장소와 일부 쿠키 삭제를 시도하지만 완전 삭제를 보장하지 않으며 카카오 로그인 쿠키는 지우지 않습니다. [실험 범위와 테스트 안내](docs/ALPHA_0.4.0.md)를 먼저 확인해 주세요.
...
0.4.0에서는 서로 다른 날의 실측과 당시 추정이 3회 이상 쌓이면 평균·최대 차이를 보여줍니다. 최근 90일 중 최대 12일을 비교한 과거 기록이며, 현재 추정의 오차 범위나 제출 안전성을 보장하는 수치는 아닙니다. [0.4.0 업데이트 범위](docs/ALPHA_0.4.0.md)에서 공급사 선택 확장과 추정 모델 변경을 확인할 수 있습니다.
...
위의 광고·추적 제한은 앱 자체에 관한 설명입니다. 실험실에서 여는 에너지톡·카카오 공식 웹 화면에는 각 서비스의 개인정보 정책이 적용되며 웹 로그인 쿠키가 남을 수 있습니다. 앱은 해당 웹사이트의 정보 처리를 통제하지 않습니다.
