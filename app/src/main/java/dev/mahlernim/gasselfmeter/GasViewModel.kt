...
class GasViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun changeProvider() = attempt {
        save(data.copy(ready = false, credentials = null))
    }
    fun forgetCredentials() = attempt { save(data.copy(credentials = null)); message = "저장한 로그인 정보를 지웠어요." }
    ...
    fun login(username: String, password: String, rememberPassword: Boolean) {
        ...
        busy = true; loginError = null; progress = "로그인하고 계약을 확인하는 중"; remember = rememberPassword
    ...
    private suspend fun syncSelected(contract: Contract) {
        val result = withContext(Dispatchers.IO) { pendingClient!!.history(contract) { text -> viewModelScope.launch { progress = text } } }
    ...
    fun erase() = attempt {
        check(!busy) { "조회가 끝난 뒤 다시 시도해 주세요." }
        clearPending(); Reminders.schedule(getApplication(), Profile()); store.erase(); data = AppData(); storageError = false
